    ...
    <div class="mb-8">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-gray-600 mt-1">Stay updated with system activities and alerts</p>
        </div>
    ...
    <div class="glass-effect rounded-2xl premium-shadow p-6 mb-8">
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div class="flex gap-2 text-sm">
          <a href="?filter=all" 
             class="px-4 py-2 rounded-lg transition-all duration-200 <?= $filter==='all' ? 'bg-indigo-600 text-white' : 'bg-white text-gray-600 hover:bg-indigo-50 hover:text-indigo-600' ?>">
             All Notifications
          </a>
          <a href="?filter=unread" 
             class="px-4 py-2 rounded-lg transition-all duration-200 <?= $filter==='unread' ? 'bg-indigo-600 text-white' : 'bg-white text-gray-600 hover:bg-indigo-50 hover:text-indigo-600' ?>">
             Unread
          </a>
          <a href="?filter=read" 
             class="px-4 py-2 rounded-lg transition-all duration-200 <?= $filter==='read' ? 'bg-indigo-600 text-white' : 'bg-white text-gray-600 hover:bg-indigo-50 hover:text-indigo-600' ?>">
             Read
          </a>
        </div>
        ...
        <?php if ($notes): ?>
          <button onclick="markAllRead()" 
                  class="px-4 py-2 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-lg text-sm hover:from-green-700 hover:to-emerald-700 transition-all duration-200 flex items-center gap-2">
            ...
            <div class="timeline-item">
              ...
              <div class="ml-6 bg-white/50 rounded-xl p-4 hover:bg-white/70 transition-all duration-200">
                ...
                <div class="flex gap-3 mt-3">
                  <?php if (!empty($n['link'])): ?>
                    <a href="<?= e($n['link']) ?>" 
                       class="inline-flex items-center gap-1 text-indigo-600 text-sm hover:text-indigo-700 hover:underline">
