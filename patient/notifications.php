...
<head>
  <meta charset="utf-8">
  <title>My Notifications - Healsync</title>
...
<body class="bg-gray-100">
  ...
  <main class="pt-20 p-6 md:ml-64 transition-all duration-300 max-w-3xl mx-auto">
    <div class="flex justify-between items-center mb-6">
      ...
      <button onclick="markAllRead()" class="px-3 py-1 bg-indigo-600 text-white rounded text-sm hover:bg-indigo-700">
        Mark All as Read
      </button>
    </div>
    ...
    <div class="flex gap-4 mb-6 border-b pb-2 text-sm">
      <a href="?filter=all" class="<?= $filter==='all'?'text-indigo-600 font-semibold':'text-gray-600 hover:text-indigo-600' ?>">All</a>
      <a href="?filter=unread" class="<?= $filter==='unread'?'text-indigo-600 font-semibold':'text-gray-600 hover:text-indigo-600' ?>">Unread</a>
      <a href="?filter=read" class="<?= $filter==='read'?'text-indigo-600 font-semibold':'text-gray-600 hover:text-indigo-600' ?>">Read</a>
    </div>
        ...
        <?php foreach ($notes as $n): ?>
          <div class="timeline-item">
            <div class="timeline-icon">
              <i data-lucide="<?= $n['is_read'] ? 'check-circle' : 'bell' ?>" class="w-4 h-4 <?= $n['is_read'] ? 'text-green-600' : 'text-indigo-600' ?>"></i>
            </div>
            <div class="ml-6">
              ...
              <div class="flex gap-3 mt-1">
                <?php if ($n['link']): ?>
                  <a href="<?= e($n['link']) ?>" class="text-indigo-600 text-sm">View Details</a>
                <?php endif; ?>
                <?php if (!$n['is_read']): ?>
                  <button onclick="markRead(<?= $n['id'] ?>)" class="text-green-600 text-sm">Mark as Read</button>
                <?php endif; ?>
              </div>
            </div>
          </div>
